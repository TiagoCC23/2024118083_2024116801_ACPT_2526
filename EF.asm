...
loop_jogo:
	...
	la $t0, answer		# Carrega o endere�o da string lida
	
loop_to_upper:
	lb $t1, 0($t0)		# L� o caracter atual
	beq $t1, $zero, fim_upper  # Se for NULL (fim), para
	beq $t1, 10, fim_upper     # Se for \n (enter), para
	
	# verifica se � min�scula
	li $t2, 97
	blt $t1, $t2, proximo_char	# se for menor que 'a', ignora
	li $t2, 122
	bgt $t1, $t2, proximo_char	# Se for maior que 'z', ignora
	
	# Se chegou aqui, � min�scula. Subtrai 32 para virar mai�scula.
	sub $t1, $t1, 32
	sb $t1, 0($t0)		# Salva a letra corrigida na mem�ria
...
loop_certo:
	...
	sb $t5 0($t3)	# a c�pia da senha fica com '*' tbm
